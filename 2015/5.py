    ...
    for i in range(len(line)):
        ...
        if i != 0:
            ...
            if (line[i-1] + line[i]) in ['ab', 'cd', 'pq', 'xy']:
                wrongsuite = True
    #fin boucle
    #print(compteurvoyelle >= 3 )
    #print(double)
    return (compteurvoyelle >= 3 and double) and not(wrongsuite)
...
def isnice2(line):
    ...
    for i in range(len(line)):
        if i != 0:
            #pour les chauvechement
            if last != line[i-1] + line[i]:
                last = line[i-1] + line[i]
                #ajout dico
